...
try {
  ...
  let opencvBinDir = tryGetOpencvBinDir()
  logDebug('require', 'adding opencv binary dir to path: ' + opencvBinDir)

  if (opencvBinDir.indexOf('.asar') > -1 && opencvBinDir.indexOf('asar.unpacked') < 0) {
    const index = opencvBinDir.indexOf('.asar');
    opencvBinDir = opencvBinDir.slice(0, index + 5) + ".unpacked" + opencvBinDir.slice(index + 5);
  }
